                        ...
                        <a type="button" class="btn btn-outline-light navigation--button"
                            ...
                            @auth
                                <form action="{{route('auth.logout')}}" method="post">
                                    @csrf
                                    <button type="submit" class="btn btn-outline-dark navigation--button" href="
                                    {{route('auth.register')}}">Logout</button>
                                </form>
                            @endauth
                    </div>
                </div>
...
<div class="col-sm order-sm-2">
    <img src="{{asset('front')}}/assets/images/team-Hos.jpg" class="img-fluid banner-img" alt="banner-image" style="width: 100%;" height="200">
</div>
    ...
    document.addEventListener( 'DOMContentLoaded', function () {
        new Splide( '.home__slider__doctors', {
            type    : 'loop',
            perPage : 4, // عدد البطاقات المعروضة في كل صفحة
            autoplay: true,
        } ).mount();
    } );
</script>
